(() => {
  ...
  try {
    ...
    data.photoJournal.forEach(section => {
      ...
      section.photos.forEach(photo => {
        ...
        const info = photo.plantInfo
          ? `<a class="plant-info-link" href="${safeUrl(photo.plantInfo)}" aria-label="View plant information and growing guide">ⓘ Plant information</a>`
          : '';
      ...
      sec.append(heading, gallery);
      frag.appendChild(sec);
    });
    el.replaceChildren(frag);
  } catch (err) {
    el.innerHTML = `<div class="v3-error"><strong>Photos could not load.</strong><br>${escapeHtml(err.message)}</div>`;
  }
  ...
  function safeUrl(s = '') {
    return encodeURI(String(s).replace(/["'<>]/g, ''));
  }
})();
